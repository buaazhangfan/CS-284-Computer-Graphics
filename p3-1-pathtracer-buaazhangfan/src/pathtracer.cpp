...
Spectrum PathTracer::estimate_direct_lighting_hemisphere(const Ray& r, const Intersection& isect) {
  ...
  for(int i = 0; i < num_samples; ++i){
    ...
    Ray r = Ray(EPS_D * wi_world + hit_p, wi_world);
    float pdf = 1.0 / (2.0 * PI);
    Intersection intersection;
    Spectrum sp(0,0,0);
    if(bvh->intersect(r, &intersection)){
      Spectrum emission = intersection.bsdf->get_emission();
      sp = (isect.bsdf->f(w_out, wi) * cos_theta(wi) * emission) / pdf;
    }
    else
      continue;
    L_out += sp;
...
Spectrum PathTracer::estimate_direct_lighting_importance(const Ray& r, const Intersection& isect) {
  ...
  for (auto s : scene -> lights)
  {
    ...
    for (int i = 0; i < num_samples; i++)
    {
      ...
      if (!bvh -> intersect(r))
      {
        L_s += isect.bsdf -> f(w_out, w_in) * cos_theta(w_in) * sp / pdf;
      }
    }
...
Spectrum PathTracer::one_bounce_radiance(const Ray&r, const Intersection& isect) {
  ...
  Spectrum PathTracer::at_least_one_bounce_radiance(const Ray&r, const Intersection& isect, int count) {
    Matrix3x3 o2w;
    make_coord_space(o2w, isect.n);
    Matrix3x3 w2o = o2w.T();
    ...
    double p = r.depth == max_ray_depth ? 1.0 : 0.7;
    bool b = coin_flip(p) && r.depth > 1;
    if(b)
    {
      ...
      Intersection intersection;
      if(bvh->intersect(shadow, &intersection))
      {
        /**
        ...
        /*
        if (count == 0)
          L_out = at_least_one_bounce_radiance(shadow, intersection, count + 1) * sp * cos_theta(w_in) / (pdf * p);
        else
        */
          L_out += at_least_one_bounce_radiance(shadow, intersection, count + 1) * sp * cos_theta(w_in) / (pdf * p);
      }
  ...
  Spectrum PathTracer::est_radiance_global_illumination(const Ray &r) {
    ...
    // For testing the effect of max_ray_depth
    ...
    if (max_ray_depth == 0)
        ...
        return zero_bounce_radiance(r,isect) + at_least_one_bounce_radiance(r,isect, count);
...
Spectrum PathTracer::raytrace_pixel(size_t x, size_t y) {
  ...
  if (num_samples == 1)
  {
    double x_p = ((float)x + 0.5) / (double)sampleBuffer.w;
    double y_p = ((float)y + 0.5) / (double)sampleBuffer.h;
    Ray r = camera -> generate_ray(x_p, y_p);
  ...
  else
  {
    for (int i = 0; i < num_samples; i++)
    {
      ...
      double x_p = (random.x + x) / (double)sampleBuffer.w;
      double y_p = (random.y + y) / (double)sampleBuffer.h;
      Ray r = camera -> generate_ray(x_p, y_p);
  ...
  */
	...
	for (int i = 0; i < num_samples; i++)
    {
        ...
        if ((i + 1) % samplesPerBatch == 0)
        {
          int n = i + 1;
          u = s1 / (double)n;
          sigma_squre = (s2 - (s1 * s1) / (double)(n)) / ((double)n - 1);
          sigma = sqrt(sigma_squre);
          I = 1.96 * (sigma / sqrt(n));
          if (I <= maxTolerance * u)
          {
