...
void DrawRend::redraw() {
  ...
  t = clock() - t;
  float internal = (float)t/CLOCKS_PER_SEC;
  cout << "The time to draw is: " << internal << "seconds" << endl;
  // draw canvas outline
...
double DrawRend::inside(double point_x, double point_y, double x, double y, double dx, double dy) {
  
  double cross_pro = - (point_x - x) * dy + (point_y - y) * dx;
...
void DrawRend::rasterize_triangle( float x0, float y0,
    ...
    int x_min = (int)floor(min(x0, min(x1, x2)));
    int x_max = (int)floor(max(x0, max(x1, x2)));
    int y_min = (int)floor(min(y0, min(y1, y2)));
    int y_max = (int)floor(max(y0, max(y1, y2)));
    
    for (int i = x_min; i <= x_max; i++)
    {
      for (int j = y_min; j <= y_max; j++)
      {
          for (int sub_row = 0; sub_row < sqrt(sample_rate); sub_row++)
          {
            for (int sub_col = 0; sub_col < sqrt(sample_rate); sub_col++)
            {
              double cell = 1.0 / sqrt(sample_rate);
              double in_1 = inside(i + (sub_col + 0.5) * cell, j + (sub_row + 0.5) * cell, x0, y0, dx0, dy0);
              double in_2 = inside(i + (sub_col + 0.5) * cell, j + (sub_row + 0.5) * cell, x1, y1, dx1, dy1);
              double in_3 = inside(i + (sub_col + 0.5) * cell, j + (sub_row + 0.5) * cell, x2, y2, dx2, dy2);
              if ((in_1 >=0 && in_2 >=0 && in_3 >=0) || (in_1 <= 0 && in_2 <= 0 && in_3 <= 0) )
              {
                  /* part 4, part 5, part 6*/
                  /*
                  double x = i + (sub_col + 0.5) * cell;
                  double y = j + (sub_row + 0.5) * cell;
                  Vector3D p_bary = Vector3D();
                  p_bary[0] = inside(x, y, x1, y1, dx1, dy1) / inside(x0, y0, x1, y1, dx1, dy1);
                  p_bary[1] = inside(x, y, x2, y2, dx2, dy2) / inside(x1, y1, x2, y2, dx2, dy2);
                  ...
                  double x_dx = x + cell;
                  double y_dx = y;
                  Vector3D p_dx_bary = Vector3D();
                  p_dx_bary[0] = inside(x_dx, y_dx, x1, y1, dx1, dy1) / inside(x0, y0, x1, y1, dx1, dy1);
                  p_dx_bary[1] = inside(x_dx, y_dx, x2, y2, dx2, dy2) / inside(x1, y1, x2, y2, dx2, dy2);
                  p_dx_bary[2] = 1 - p_dx_bary[0] - p_dx_bary[1];
                  
                  // x, y + 1
                  double x_dy = x;
                  double y_dy = y + 1;
                  Vector3D p_dy_bary = Vector3D();
                  p_dy_bary[0] = inside(x_dy, y_dy, x1, y1, dx1, dy1) / inside(x0, y0, x1, y1, dx1, dy1);
                  p_dy_bary[1] = inside(x_dy, y_dy, x2, y2, dx2, dy2) / inside(x1, y1, x2, y2, dx2, dy2);
                  p_dy_bary[2] = 1 - p_dy_bary[0] - p_dy_bary[1];
                  // cout << p_dx_bary[0] << " " << p_dx_bary[1] << " " << p_dx_bary[2] << endl;
                  Color bary_c = tri -> color(p_bary, p_dx_bary, p_dy_bary, sampleParams);
                  samplebuffer[j][i].fill_color(sub_row, sub_col, bary_c);
                  */
                  samplebuffer[j][i].fill_color(sub_row, sub_col, color);
              }
            }
          }
       }
 ...
 /* Part 1
  void DrawRend::rasterize_triangle( float x0, float y0,
    ...
    for (int i = x_min; i <= x_max; i++) {
      for (int j = y_min; j <= y_max; j++) {
        bool in = true;
        double in1 = inside(i + 0.5, j + 0.5, x0, y0, dx0, dy0);
        double in2 = inside(i + 0.5, j + 0.5, x1, y1, dx1, dy1);
        double in3 = inside(i + 0.5, j + 0.5, x2, y2, dx2, dy2);
        if ((in1 >= 0 && in2 >= 0 && in3 >= 0) || (in1 <= 0 && in2 <= 0 && in3 <= 0))
        {
