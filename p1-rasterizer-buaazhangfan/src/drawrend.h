...
class DrawRend : public Renderer {
 public:
  ...
  void rasterize_line( float x0, float y0,
                       float x1, float y1,
                       Color color);
  double inside(double point_x, double point_y, double x, double y, double dx, double dy);
...
private:
  ...
  struct SampleBuffer {
    ...
    Color get_pixel_color() {
      ...
      vector<unsigned char>sum_sub(3, 0);
      int r = 0;
      int g = 0;
      int b = 0;
      for (int i = 0; i < samples_per_side; ++i)
      ...
      sum_sub[0] = (unsigned char)(r / (samples_per_side * samples_per_side));
      sum_sub[1] = (unsigned char)(g / (samples_per_side * samples_per_side));
      sum_sub[2] = (unsigned char)(b / (samples_per_side * samples_per_side));
      
      return Color(sum_sub.data());
