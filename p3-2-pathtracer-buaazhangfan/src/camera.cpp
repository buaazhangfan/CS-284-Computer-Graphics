...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
    ...
    double dirX = bottomLeft.x + x * (topRight.x - bottomLeft.x);
    double dirY = bottomLeft.y + y * (topRight.y - bottomLeft.x);
    double dirZ = -1;

    Vector3D dir = Vector3D(dirX, dirY, dirZ);
    double pLens_x = lensRadius * sqrt(rndR) * cos(rndTheta);
    double pLens_y = lensRadius * sqrt(rndR) * sin(rndTheta);

    Vector3D pLens = Vector3D(pLens_x, pLens_y, 0);

    Vector3D pFocus = dir * ((-1.0 * focalDistance) / dir.z);
    Ray sample_r = Ray(pLens, pFocus - pLens);
    sample_r.d.normalize();
    sample_r.o = c2w * sample_r.o + pos;
    sample_r.d = c2w * sample_r.d;
    sample_r.min_t = nClip;
    sample_r.max_t = fClip;
    return sample_r;
}
